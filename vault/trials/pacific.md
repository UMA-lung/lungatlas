...
pending:
  ...
  - "PD-L1陰性/<1%サブグループのHRは解析時点で変動：2018年OS解析時点でHR 1.36（95%CI 0.79-2.34、日本肺癌学会GL引用）、4年フォローHR 1.05、5年フォローHR 1.15。いずれも探索的事後解析かつCIが1をまたぐため、経時的な傾向としてのみ捉え、確定的な効果差とは解釈しない"
  - "PACIFIC-2（同時併用、陰性試験）データはvault/trials/pacific2.md参照"
publish_checklist: []
...
- **日本人サブグループの安全性**（Murakami S, et al. ESMO Asia 2017 Congress, 403O）：放射線肺臓炎（全Grade）デュルバルマブ群73.6% vs プラセボ群60.0%、Grade3-4は5.6% vs 2.5%、Grade5は1.4% vs 2.5%。グローバル集団（32.8% vs 23.5%）より高い発現率である点に留意
- **新規脳転移発生率**（NCCN Guidelines Insights, JNCCN 2025;23(9)引用）：デュルバルマブ群6.5% vs プラセボ群11.8%（5年解析時点）
